...
class TextractAnalyzer:
    ...
    def analyze_document(
        self, image_source: str, feature_types: List[str] = None
    ) -> Dict:
        """
    ...
    def extract_tables(self, blocks: List[Dict]) -> List[List[List[str]]]:
        ...
        for table_block in table_blocks:
            ...
            cells = [
                ...
                and block.get("Relationships", [{}])[0].get("Ids", [None])[0]
                == table_block["Id"]
            ]
                ...
                for relationship in key_block["Relationships"]:
                    if relationship["Type"] == "VALUE":
                        for value_id in relationship["Ids"]:
                            ...
                            if (
                                value_block
                                and value_block["BlockType"] == "KEY_VALUE_SET"
                            ):
                                # Get text from child blocks
                                for child_id in value_block.get("Relationships", [{}])[
                                    0
                                ].get("Ids", []):
                                    child_block = next(
    ...
    def parse_receipt(self, text):
        ...
        normalized = re.sub(r"\n(?!\d)", " ", text)
        normalized = re.sub(r"A ", "", normalized)  # Clean up multiple newlines
        ...
        for match in pattern.finditer(normalized):
            ...
            products.append({"name": name, "quantity": qty, "price": price})
...
def main():
    ...
    parser.add_argument(
        "--region", help="AWS region (overrides AWS_DEFAULT_REGION from .env)"
    )
    parser.add_argument(
        "--profile", help="AWS profile name (overrides .env credentials)"
    )
    parser.add_argument(
        "--features",
        nargs="+",
        choices=["TABLES", "FORMS"],
        default=["TABLES", "FORMS"],
        help="Feature types to detect",
    )
